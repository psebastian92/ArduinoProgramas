...
#include <Wire.h>               // Incluye la librería para I2C
#include <LiquidCrystal_I2C.h>  // Incluye la librería para LCD I2C
...
// Inicializa el objeto LiquidCrystal_I2C (ajusta la dirección y el tamaño según tu configuración)
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {                         // Verifica que estés conectado a WiFi
    ...
    // Determina el estado del nivel de sonido
    String estado;
    if (nivelSonidoDB < 60) {
      estado = "Estable";
    } else if (nivelSonidoDB < 80) {
      estado = "Alto";
    } else {
      estado = "Peligroso";
    }
    ...
    lcd.print(estado);    // Muestra el estado
