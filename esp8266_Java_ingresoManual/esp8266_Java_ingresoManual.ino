...
const char* server = "http://192.168.0.14:8080/WiFiBD/RecibirDatosESP8266";  // URL completa del servlet
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {  // Verifica que estés conectado a WiFi
    ...
    if (Serial.available() > 0) {
      ...
      String inputString = Serial.readStringUntil('\n'); // Lee el valor ingresado hasta un salto de línea
      double nivelSonidoDB = inputString.toDouble(); // Convierte la cadena a un valor numérico

      // Imprime el valor leído para verificar
      Serial.print("Nivel de sonido ingresado: ");
